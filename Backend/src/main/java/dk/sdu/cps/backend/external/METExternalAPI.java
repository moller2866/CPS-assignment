...
public class METExternalAPI {
    ...
    public METExternalAPI(
            RestTemplate restTemplate,
            WeatherMeasurementRepositoryImpl weatherRepository,
    ...
    public void FetchAndSaveMeasurement(LocationDTO location) {
        String url =
                "https://api.met.no/weatherapi/nowcast/2.0/complete.json?lat="
        ...
        LocalDateTime time =
                Instant.parse(firstEntry.time()).atZone(ZoneId.systemDefault()).toLocalDateTime();
        Details details = firstEntry.data().instant().details();
        Next_1_hours next1Hours = firstEntry.data().next_1_hours();
        IWeatherMeasurementDTO latestWeatherData =
                weatherRepository.getLatestFromLocation(location.getName());
        if (latestWeatherData != null && latestWeatherData.getTimeStamp().equals(time)) {
            return;
        }
        WeatherMeasurementDTO weatherData =
                getWeatherMeasurementDTO(time, details, next1Hours, location.getName());
        weatherRepository.create(weatherData);
    }

    private static WeatherMeasurementDTO getWeatherMeasurementDTO(
            LocalDateTime time, Details details, Next_1_hours next1Hours, String locationName) {
